...
function replaceTextareasWithDivsForCapture(container) {
  ...
  container.querySelectorAll("textarea").forEach((textarea) => {
    ...
    // 🔧 Usamos getBoundingClientRect() (siempre devuelve píxeles reales)
    // en vez de getComputedStyle().width (que a veces puede devolver un
    // valor no resuelto, como "100%", rompiendo el cálculo de ajuste de
    // línea del texto).
    const totalWidth = textarea.getBoundingClientRect().width || 0;
